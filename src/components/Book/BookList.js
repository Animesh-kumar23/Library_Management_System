...
const BookList = () => {
  ...
  const handleReturn = async (bookId) => {
    try {
      ...
      if (token) {
        ...
        await api.post(
          'https://library-management-system-3-ciao.onrender.com/api/transaction/return',
          { bookId, userId },
          {
            headers: {
  ...
  const handleDeleteBook = async (bookId) => {
    try {
      const token = localStorage.getItem('token');
      if (token) {
        await api.delete(`https://library-management-system-3-ciao.onrender.com/api/books/${bookId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
